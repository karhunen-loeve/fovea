...
/// Disjoint-set forest over `u32` labels.
///
...
/// `u32` is the label width of
/// [`LabelPixel::MAX_LABEL`](crate::pixel::LabelPixel); exhaustion of
/// the `u32` space is reported by [`make_set`](UnionFind::make_set),
...
impl UnionFind {
    ...
    ///
    /// The first call returns `Some(1)`; the `k`th call returns
    /// `Some(k)`. Returns `None` once the `u32` label space is spent
    /// (the `(u32::MAX + 1)`-th call) — the engine reports that as
    /// [`LabelOverflow`](crate::Error::LabelOverflow).
    pub(super) fn make_set(&mut self) -> Option<u32> {
        let id = u32::try_from(self.parent.len()).ok()?;
        self.parent.push(id);
        self.rank.push(0);
        Some(id)
    }
    ...
    #[cfg(test)]
    pub(super) fn raw_parent(&self, x: u32) -> u32 {
...
mod tests {
    ...
    fn first_make_set_returns_one() {
        ...
        assert_eq!(uf.make_set(), Some(1));
        assert_eq!(uf.make_set(), Some(2));
        assert_eq!(uf.make_set(), Some(3));
    ...
    fn union_chain() {
        let mut uf = UnionFind::with_capacity(0);
        let ids: Vec<u32> = (0..5).map(|_| uf.make_set().unwrap()).collect();
    ...
    fn path_compression_flattens_chain() {
        ...
        let ids: Vec<u32> = (0..6).map(|_| uf.make_set().unwrap()).collect();
    ...
    fn union_by_rank_keeps_tree_balanced() {
        ...
        let mut uf = UnionFind::with_capacity(0);
        let ids: Vec<u32> = (0..64).map(|_| uf.make_set().unwrap()).collect();
        for w in ids.windows(2) {
            uf.union(w[0], w[1]);
        }
